...
# util.py
#
# This script provides a collection of universal utility classes and functions
# used throughout the PyCHAMP model. It includes a custom MESA scheduler,
# data collection helpers, a time recorder, and statistical indicator tools.
#
# This version has been unified to support all policy scenarios by including
# conditional logic for seniority-based agent activation.
#
...
class BaseSchedulerByTypeFiltered(BaseScheduler):
    ...
    def do_each(self, method, agent_keys=None, shuffle=False, agt_type=None):
        """
        Executes a method for each agent, with optional filtering and sorting.
        """
        if agent_keys is None:
            agent_keys = self.get_agent_keys()
        
        # Filter agents by their type if specified
        if agt_type is not None:
            agent_keys = [i for i in agent_keys if self._agents[i].agt_type == agt_type]
            
            # If stepping Behavior agents, check for seniority and sort accordingly
            if agt_type == "Behavior" and agent_keys and hasattr(self._agents[agent_keys[0]], 'seniority_id'):
                agent_keys.sort(key=lambda k: int(getattr(self._agents[k], 'seniority_id')))
        ...
        for agent_key in agent_keys:
            if agent_key in self._agents:
                ...
                if agt_type == "Behavior":
                    ...
                    # print(f"Processing Seniority ID {seniority_id}, Accumulated pumping: {self.model.accumulated_withdrawal}")

# --- 3. Data Collection Helper Functions ---
