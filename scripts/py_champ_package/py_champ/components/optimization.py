...
# optimization.py
#
# This script defines the Optimization class, which formulates and solves the
# farmer agent's decision-making problem as a nonlinear mixed-integer program.
# The objective is to maximize profit over a defined planning horizon.
#
...
# --- 2. Define the Optimization Class ---
class Optimization:
    """
    ...
    def setup_ini_model(
        ...
        #temp_crop_cost
        temp_crop_cost = m.addMVar((n_s, n_c, n_h), vtype="C", name="temp_crop_cost(1e4bu)", lb=0, ub=inf)

        ## Record variables
        self.vars_["irr_depth"] = irr_depth
        self.vars_["temp_crop_cost"] = temp_crop_cost
        self.vars_["v"] = v
        self.vars_["y"] = y
        self.vars_["e"] = e
    ...
    def setup_constr_field(
        ...
        **kwargs:
        ...
        temp_crop_cost = m.addMVar((n_s, n_c, n_h), vtype="C", name=f"{fid}.temp_crop_cost($1e4)", lb=0, ub=inf)
        ...
        m.addConstr((temp_crop_cost == i_crop * unit_area / 0.404686 * 1e-4), name=f"c.{fid}.temp_crop_cost") # per ac to per ha
        ...
        self.vars_[fid]["temp_crop_cost"] = temp_crop_cost
        self.vars_[fid]["field_type"] = field_type
        self.vars_[fid]["perceived_precipitation"] = prec_aw_
        self.n_fields += 1
    ...
    def setup_constr_finance(self, finance_dict):
        ...
        crop_profit = {c: finance_dict["crop_price_avg"][c]  for c in crop_options}
        # print("crop_profit:", crop_profit)
        crop_variable_cost = {c: finance_dict["crop_variable_cost_avg"][c] for c in crop_options}
        # print("crop_variable_cost:", crop_variable_cost)
        crop_fixed_cost = {c: finance_dict["crop_fixed_cost_avg"][c] for c in crop_options}
        # print("crop_fixed_cost:", crop_fixed_cost)
        cost_tech = np.array(
        ...
        v = vars_['v']     # (n_h) [m-ha]
        temp_crop_cost = vars_["temp_crop_cost"]
        
        cost_e = m.addMVar((n_h), vtype="C", name="cost_e(1e4$)", lb=0, ub=inf)
        # rev_ = m.addMVar((n_h), vtype="C", name="rev_(1e4$)", lb=-inf, ub=inf) # previous definition of revenue
        rev = m.addMVar((n_h), vtype="C", name="rev(1e4$)", lb=-inf, ub=inf)
        fixed_crop_production_cost = m.addMVar((n_h), vtype="C", name="fixed_crop_production_cost(1e4$)", lb=0, ub=inf)
        variable_crop_production_cost = m.addMVar((n_h), vtype="C", name="variable_crop_production_cost(1e4$)", lb=0, ub=inf)

        # annual_tech_cost = 0
        annual_tech_change_cost = 0
        annual_crop_change_cost = 0
        annual_tech_cost_ = {fid: {} for fid in field_ids}
        for fid in field_ids:
            i_te = vars_[fid]["i_te"]  # One-dimensional array, the same for all splits
            i_rainfed = vars_[fid]["i_rainfed"]  # Dimensioned as (n_s, n_c, 1) for rainfed status per split and crop
        
            # Initialize the array to hold split-level costs for the current field
            for si in range(n_s):
                # Define a variable for tech cost at each split if technology is used and not rainfed
                annual_tech_cost_[fid][si] = m.addVar(vtype="C", name=f"annual_tech_cost_{fid}_{si}")
                
                # Sum the rainfed status over all crops for the current split
                # Using the max to determine if any crop in the split is not rainfed (i.e., might need irrigation technology)
                is_rainfed_split = m.addVar(vtype="B", name=f"is_rainfed_{fid}_{si}")
                # Extract variables from i_rainfed slice and convert them into a list
                rainfed_vars = [i_rainfed[si, ci, 0] for ci in range(i_rainfed.shape[1])]
                
                # Apply gp.max_ to a list of Gurobi variables
                m.addGenConstrMax(is_rainfed_split, rainfed_vars, name=f"def_is_rainfed_{fid}_{si}")
        
                # Technology cost application condition: apply if not rainfed
                m.addConstr(annual_tech_cost_[fid][si] == i_te * cost_tech * (1 - is_rainfed_split),
                            name=f"c.tech_cost_{fid}_{si}")
        
            # Sum up the costs from all splits in the current field
            annual_tech_cost_[fid]['total'] = m.addVar(vtype="C", name=f"annual_tech_cost_{fid}_total")
            m.addConstr(annual_tech_cost_[fid]['total'] == gp.quicksum(annual_tech_cost_[fid][si] for si in range(n_s)),
                        name=f"c.tech_cost_{fid}")
        
        # Total technology cost across all fields
        annual_tech_cost = m.addVar(vtype="C", name="annual_tech_cost_total")
        m.addConstr(annual_tech_cost == gp.quicksum(annual_tech_cost_[fid]['total'] for fid in field_ids),
                    name="c.total_tech_cost")
        ...
        # pumping fee
        pumping_fee = finance_dict.get('pumping_fee', 0) or 0
        ...
        m.addConstr(fixed_crop_production_cost == gp.quicksum(temp_crop_cost[i, j, :] * 
                                                              crop_fixed_cost[c]for i in range(n_s) for j, c in enumerate(crop_options)),
                                                              name="c.fixed_crop_production_cost",)
        m.addConstr(variable_crop_production_cost == gp.quicksum(temp_crop_cost[i, j, :] * 
                                                              crop_variable_cost[c]for i in range(n_s) for j, c in enumerate(crop_options)),
                                                              name="c.variable_crop_production_cost",)
        # production cost based on acreage
        # fixed_crop_production_cost_ = {}
        # for fid in field_ids:
        #     i_crop = vars_[fid]["i_crop"] # (n_s, n_c, 0)
        #     # fixed_crop_production_cost_ = vars_[fid]["fixed_crop_production_cost_"]
        #     fixed_crop_production_cost_[fid]=sum(
        #         i_crop[i, j, :] * crop_cost[c] *  * 10^-4 asad
        #         for i in range(n_s)   # Assuming n_s is the number of splits
        #         for j, c in enumerate(crop_options))   # Assuming n_c is the number of crop types
                
        # # m.addConstr(fixed_crop_production_cost == 
        # #             gp.quicksum(i_crop[i,:,:] * crop_cost[c] 
        # #             for i in range(n_s)
        # #             for c in enumerate(crop_options)), name ="c.fixed_production_cost(1e4$)" )
        # m.addConstr(fixed_crop_production_cost == gp.quicksum(fixed_crop_production_cost_[fid] for fid in field_ids), name="c.fixed_production_cost(1e4$)")

        vars_["rev"] = rev
        vars_["fixed_production_cost"] = fixed_crop_production_cost
        vars_["variable_production_cost"] = variable_crop_production_cost
        vars_["cost_e"] = cost_e
        vars_['cost_p'] = cost_p
    ...
    def setup_obj(self, alpha_dict=None):
        ...
        # Check the selected target exists
        if target not in eval_metric_vars:
            raise ValueError(f"Target metric '{target}' is not supported.")
        ...
        metric_var = eval_metric_vars[target]
        avg_metric = gp.quicksum(metric_var[h] for h in range(n_h)) / n_h
        m.setObjective(avg_metric - penalty, gp.GRB.MAXIMIZE)      
    ...
    def finish_setup(self, display_summary=True):
        ...
        m.addConstr(temp_crop_cost == get_sum(fids, vars_, "temp_crop_cost"), name="c.temp_crop_cost($1e4)")
        ...
        fixed_crop_production_cost = vars_["fixed_production_cost"]
        variable_crop_production_cost = vars_["variable_production_cost"]
        if current_year >= 2002:
            m.addConstr((profit == (rev - fixed_crop_production_cost - variable_crop_production_cost - 
                                    cost_e - cost_p - annual_cost)/n_f), name="c.profit")
        else:
            m.addConstr((profit == (rev - fixed_crop_production_cost - variable_crop_production_cost- 
                                    cost_e - annual_cost)/n_f), name="c.profit")
    ...
    def solve(
        ...
        **kwargs : **kwargs
        ...
        for k, v in gurobi_kwargs.items():
            m.setParam(k, v)    
        m.setParam('IntFeasTol', 1e-9)  # Set a stricter tolerance to enforce binary constraints
        m.optimize()
        ...
        ###################################
            """
        # Satisfaction:\n{sas}\n (was at the end of the report above previously)
